...
Okay now, we are going to use the information in the candidate gene table to attach a beta-coedifcceitn to each SNV
...
- `interval`: interval coordinates (`13865698-15025162`), correspond to `startPOS`-`endPOS` values in the finemap_data

- `MARKER`: SNV ID in each candidate gene in `CHROM_POS` format (e.g. `V_14554027`)

Relevant columns in the finemap data:

- `trait_value`: name of the trait of interest (e.g. `2_4_D`)