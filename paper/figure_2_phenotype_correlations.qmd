---
title: "Figure 02. Correlations between differences in susceptibility to toxicant conditions"
execute:
  warning: false
  message: false
---
...
inhibition_data <- data.table::fread(file.path("data/processed/phenotypes/toxicant_inhibition_summary.csv"))

# Load correlation summaries from analysis script
pheno_corr_all_avg <- readRDS(file.path("data/processed/phenotypes/pheno_corr_all_avg.Rds"))
class_correlation_summary <- data.table::fread(file.path("data/processed/phenotypes/class_correlation_summary.csv"))
```
...
Report within- and between-group correlation summaries
```{r}
...
tox_cols <- tox_data %>%
  ...
  dplyr::select(drug, nice_drug_label2, big_class, short_moa_class, col) %>%
  dplyr::rename(MoA = short_moa_class, Use = big_class)
...
# Join inhibition data with tox_cols for annotation
tox_cols_with_inhibition <- tox_cols %>%
  dplyr::left_join(
    inhibition_data %>% dplyr::select(nice_drug_label2, mean_pct_inhibition),
    by = "nice_drug_label2"
  )

moa_anno_df <- tox_cols_with_inhibition %>%
  ...
  # Rename mean_pct_inhibition for display
  dplyr::rename(`% Growth Inhib.` = mean_pct_inhibition) %>%
  # Reorder columns: Use, MoA, % Growth Inhib. (right to left on heatmap)
  dplyr::select(`% Growth Inhib.`, MoA, Use)

# Create annotation colors for Use (categorical)
...
# Higher % inhibition = darker, anchored to 0-100% range
# Using gray90 (light) to gray20 (dark)
