...
# Format inhibition data as "mean ± SD" rounded to 1 decimal place
inhibition_formatted <- inhibition_data %>%
  dplyr::mutate(
    `% Growth Inhib.` = paste0(
      round(mean_pct_inhibition, 1),
      " \u00b1 ",
      round(sd_pct_inhibition, 1)
    )
  ) %>%
  dplyr::select(drug, `% Growth Inhib.`)
...
tox_table_pre <- tox_metadata %>%
  ...
  dplyr::left_join(inhibition_formatted, by = "drug") %>%
  dplyr::select(-source, -cat_num, -drug) %>%
  dplyr::rename(
...
tox_table_ft <- flextable::flextable(
  ...
  flextable::align(j = "% Growth Inhib.", align = "center") %>%
  flextable::align(j = "References", align = "left") %>%
