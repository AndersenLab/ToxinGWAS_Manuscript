...
candidate_flex_table_pre <- candidate_pre_table %>%
  ...
  # join the max log10 p-value to the candidate gene table
  dplyr::left_join(candidate_max_log10p, by = c("WBGeneID")) %>%
  dplyr::distinct() %>%
...
candidate_ft <- candidate_flex_table_pre %>%
  ...
  flextable::set_header_labels(
    ...
    max_marker_ld = "R2",
    max_marker_maf = "MAF",
    drug_marker = "Interval",
    rank = "Interval Rank"
  ) %>%
  # make the gene name bold and italic
  flextable::italic(j = 1, i = ~ !is.na(SYMBOL), part = "body") %>%
  # center the rank column
  flextable::align(j = "rank", align = "center", part = "all") %>%
  # replace NA values with "N/a"
  flextable::compose(j = "rank", value = as_paragraph(ifelse(is.na(rank), "N/a", rank))) %>%
  # add vertical line before interval column
  flextable::vline(i = 1, j = 5, part = "header", border = officer::fp_border(color = "black")) %>%
  flextable::vline(j = 5, part = "body", border = officer::fp_border(color = "black")) %>%
